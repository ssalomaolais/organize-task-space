...
const TaskBasicTab = ({
  ...
  let wStack = stack;
  if (user?.stack != "DNW" && formData.title === "") {
    wStack = stack.filter((item) => user?.stack.includes(item.value));
  }
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="md:col-span-2 flex flex-col justify-end h-full">
          ...
          <Input id="title" value={formData.title} onChange={(e) => onInputChange("title", e.target.value)} placeholder="Digite o título da tarefa" required />
        </div>
        <div className="md:col-span-2 flex flex-row gap-2 items-end">
          <div className="flex-1 flex flex-col">
            <Label htmlFor="subtitle">Tags</Label>
            <Input id="subtitle" value={formData.subtitle} onChange={(e) => onInputChange("subtitle", e.target.value)} placeholder="Ex: #Jornada Cloud, #Aculturamento de IA " />
          </div>
          <div className="w-64 flex flex-col relative">
            ...
            {formData.origin !== undefined && formData.origin !== null && (
              <button type="button" className="absolute right-2 top-8 text-gray-400 hover:text-gray-600" onClick={() => onInputChange("origin", null)} title="Limpar seleção">
                ×
              </button>
            )}
          </div>
        ...
        <div className="md:col-span-2 flex flex-col justify-end h-full">
          ...
          <Textarea id="description" value={formData.description} onChange={(e) => onInputChange("description", e.target.value)} placeholder="Descreva a tarefa em detalhes" rows={6} />
        </div>
        <div className="md:col-span-2 flex flex-col justify-end h-full">
        ...
        <div className="flex flex-col justify-end h-full">
          ...
          <Input id="responsible" value={formData.responsible} onChange={(e) => onInputChange("responsible", e.target.value)} placeholder="Nome do responsável" required />
        </div>

        <div className="flex items-end gap-2 h-full">
          <div className="flex-1 flex flex-col justify-end h-full">
            <Label htmlFor="stack">Comunidade *</Label>
            <Select onValueChange={(value) => onInputChange("stack", value)} value={formData.stack || ""}>
              <SelectTrigger>
        ...
        <div className="flex flex-col justify-end h-full">
          ...
          <Input id="hours" type="number" min="0" value={formData.hours || 0} onChange={(e) => onInputChange("hours", parseInt(e.target.value) || 0)} placeholder="Ex: 40" />
        </div>

        <div className="flex flex-col justify-end h-full">
          <Label htmlFor="people">Número de Participantes</Label>
          <Input id="people" type="number" min="0" value={formData.people || 0} onChange={(e) => onInputChange("people", parseInt(e.target.value) || 0)} placeholder="Ex: 2" />
        </div>
        ...
        <div className="flex items-end gap-2 h-full">
          <div className="flex-1 flex flex-col justify-end h-full">
            ...
            <Select onValueChange={(value) => onInputChange("event_type", value)} value={formData.event_type || ""}>
              <SelectTrigger>
